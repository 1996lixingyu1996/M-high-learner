#### Expe 4 multiple mediators sample size = 1000####
#####  Single Mediator Global #######
##### Experiment 4 sample size 1000 ######
## Mediator Model  10 mediators
##### Indirect experiment ########
...
f_exp1 <- function(seed){
  ...
  for (i in 1:n){
    if(W[i]==1){
      ...
      Y[i] = 1 + 0.5*X[i,1]+0.5*X[i,2] +rnorm(1,0,0.01)+ 0.5*X[i,3] +0.5*X[i,4]+0.1*sum(M[i,]) +0.5*W[i]*X[i,1]+0.5*W[i]*X[i,2]
    }else{
      for(j in 1:q){
  ...
  rf_y_1 = rfsrc(as.formula(formula_str_M),data = data1)

  n = dim(data)[1]
  n_mediator = q#length(name)#length(colnames(data_new)[61:203])
  ...
  pval_flag_M = 1
  for (j in 2:7){
    x_name = paste0("Cov",1:50)#c("age1c","lipid","bmi1c","cursmk1") #colnames(data_new)[c(1,3:6,9:20)]#paste0("Cov",1:100)
  ...
  data_save_path = paste0(".../expe4_continuous_1000_multi/data_seed_",seed,".csv")
  rio::export(data, data_save_path)
  result = list()
  save_path = paste0(".../expe4_continuous_1000_multi/seed_",seed,".rds")
...
library(parallel)
library(MASS)
my_new_folder = ".../expe4_continuous_1000_multi"
