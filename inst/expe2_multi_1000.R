#### Expe 2 multiple mediators sample size = 1000####
#####  Part #######
##### Experiment 2 sample size 1000 ######
##### Indirect experiment ########
...
f_exp1 <- function(seed){
  ...
  if (is.null(beta_mat)) {
    ...
    beta_mat[1, ] <- 0.5 + seq(-0.15,0.15,by=(0.3/9))     # x1
    beta_mat[2, ] <- 0.5 + seq(0.15,-0.15,by=-(0.3/9))   # x2
    beta_mat[3, ] <- 1  + seq(-0.15,0.15,by=(0.3/9)) # x3
    beta_mat[4, ] <- 1  + seq(0.15,-0.15,by=-(0.3/9))  # x4
  }
  ...
  for (i in 1:n){
    if(X[i,1]>0&X[i,2]>0&W[i]==1){
      for(j in 1:q){
        M[i,j] = M[i,j]+1*X[i,1]*W[i] +1*X[i,2]*W[i] + rnorm(1,0,0.01)
      }
      Y[i] = 1 + 0.5*X[i,1]+0.5*X[i,2] +rnorm(1,0,0.01)+ 0.5*X[i,3] +0.5*X[i,4]+0.1*sum(M[i,])+1*X[i,1]*W[i] +1*X[i,2]*W[i]
    }else{
      for(j in 1:q){
  ...
  propensity_score_formula = paste("TRT~", paste(covariate_names, collapse = " + "))
...
library(parallel)
library(MASS)
my_new_folder = ".../expe2_continuous_1000_multi"
