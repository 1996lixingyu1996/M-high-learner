#### Expe 5 multiple mediators sample size = 1000####
#####   #######
##### Experiment 5 sample size 1000 ######
...
f_exp1 <- function(seed){
  ...
  for (i in 1:n){
    if(W[i]==1){
      for(j in 1:q){
        M[i,j] = 1*X[i,1]*W[i] +1*X[i,2]*W[i]+M[i,j] + rnorm(1,0,0.01)
      }
      Y[i] = 1 + 0.5*X[i,1]+0.5*X[i,2] +rnorm(1,0,0.01)+ 0.5*X[i,3] +0.5*X[i,4]
    }else{
      for(j in 1:q){
  ...
  data_save_path = paste0("/expe5_continuous_1000_multi/data_seed_",seed,".csv")
  rio::export(data, data_save_path)
  result = list()
  save_path = paste0("/expe5_continuous_1000_multi/seed_",seed,".rds")
...
library(parallel)
library(MASS)
my_new_folder = ".../expe5_continuous_1000_multi"
