#### Expe 1 multiple mediators sample size =1000####
#####  All #######
##### Experiment 1 sample size 1000 ######
##### Indirect experiment ########
...
f_exp1 <- function(seed){
  ...
  if (is.null(beta_mat)) {
    ...
    beta_mat[1, ] <- 0.5 + seq(-0.15,0.15,by=(0.3/9))     # x1 系数
    beta_mat[2, ] <- 0.5 + seq(0.15,-0.15,by=-(0.3/9))   # x2 系数
    beta_mat[3, ] <- 1  + seq(-0.15,0.15,by=(0.3/9)) # x3 系数
    beta_mat[4, ] <- 1  + seq(0.15,-0.15,by=-(0.3/9))  # x4 系数
  }
  ...
  dis3 = (dis3)**2
  tsne_result = Rtsne::Rtsne(dis3,dims=2,is_distance=TRUE,verbose=FALSE,max_iter = 2500, theta = 0)
  ...
  pval_flag_Y = 1
  pval_flag_M = 1
  profile_index_M = NULL
  profiles_save_M = NULL
